...
def check_ips_by_flows(ips_by_flows):
    correct_hash = b'\xd6u\x96a\x104!\x16\xd3u!\xa9C}`\xd5'
    actual = prep_list(ips_by_flows).encode("utf-8")
    if check(actual, correct_hash):
...
def check_ips_by_volume(ips_by_volume):
    correct_hash = b'oz\xdf\xf8\x1d\xd8G\xbe\x8d\\\xea\xef\xb7\xca$T'
    actual = prep_list(ips_by_volume).encode("utf-8")
    if check(actual, correct_hash):
...
def check_ports_by_flows(ports_by_flows):
    correct_hash = b'\x898\xb3yYT&\xfck\x0b\x94\xc1\x87\xde\xd7\xc9'
    actual = prep_list(ports_by_flows).encode("utf-8")
    if check(actual, correct_hash):
...
def check_ports_by_volume(ports_by_volume):
    correct_hash = b'\xec\xbbL\xda\xb5c[\xe0zz\x91\xe6\x8b\x86\x05\xc0'
    actual = prep_list(ports_by_volume).encode("utf-8")
    if check(actual, correct_hash):
...
def check_longest_aspath(longest_aspath):
    correct_hash = b'\xb1dd\x8dq\xbe\xf8\x8e\xd5\xfez\xb7q\x02\xca\xc3'
    if check(longest_aspath.encode("utf-8"), correct_hash):
